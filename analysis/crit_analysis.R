...
compute_choice_props <- function(dd,groups){
  dd %>%
...
compute_mean_choice_props <- function(props,groups){
  props %>%
    ...
    pivot_wider(names_from = type,
                values_from = c(m_prop,sd,ci_lower,ci_upper))
}

# Analysis by set ==========================================================================================
d_att_choice_by_set_dist <- compute_choice_props(d_att_choice,groups=vars(set,distance,type))
d_att_choice_mean_by_set_dist <- compute_mean_choice_props(d_att_choice_by_set_dist,groups=vars(set,distance,type)) %>%
...
ggplot(d_att_choice_mean_by_set_dist,aes(m_prop_worst,m_prop_best,col=option))+
...
ggsave(filename = here("analysis","plots","crit_mean_props_by_set.jpeg"),width=8,height=7)

# analysis collapsed collapsed ===============================================================
d_att_choice_collapsed_by_dist <- d_att_choice %>%
  compute_choice_props(groups=vars(distance,type))
d_att_choice_mean_collapsed_by_dist <- compute_mean_choice_props(d_att_choice_collapsed_by_dist,
                                                                 groups=vars(distance,type))
d_att_choice_mean_collapsed_by_dist %>%
  mutate(option=factor(option,levels=c("t","c","d"))) %>%
  arrange(distance,option) %>%
  select(distance, option, m_prop_best, sd_best, ci_lower_best, ci_upper_best)
d_att_choice_mean_collapsed_by_dist %>%
  mutate(option=factor(option,levels=c("t","c","d"))) %>%
  arrange(distance,option) %>%
  select(distance, option, m_prop_worst, sd_worst, ci_lower_worst, ci_upper_worst)

# plot collapsed analysis by condition ===============================================================
d_att_choice_by_cond_dist <- compute_choice_props(d_att_choice,groups=vars(distance,type,bw_cond))

d_att_choice_mean_by_cond_dist <- compute_mean_choice_props(d_att_choice_by_cond_dist,
                                                            groups=vars(distance,type,bw_cond))

d_att_choice_mean_by_cond_dist %>%
  mutate(distance=factor(str_glue("{distance}% TDD"),
                         levels=c("2% TDD","5% TDD","9% TDD","14% TDD")),
         option=factor(option,levels = c("t","c","d"),
                       labels=c("target","competitor","decoy"))) %>%
  ggplot(aes(m_prop_worst,m_prop_best,col=option,shape=option))+
  geom_point(size=1.75,alpha=1)+
  scale_shape_manual(name="",values=c(0,2,1)) +
  scale_color_manual(name="",values=c("#FF0000","#0000FF","#006400"))+
  coord_fixed(xlim=c(0,.8),ylim=c(0,.8))+
  scale_x_continuous(labels=c("0",".2",".4",".6",".8"))+
  scale_y_continuous(labels=c("0",".2",".4",".6",".8"))+
  ggsci::scale_color_startrek(name="")+
  labs(x="p(worst)",y="p(best)")+
  facet_grid(distance~bw_cond)+
  ggthemes::theme_few()+
  theme(text=element_text(size=16),
        ...
        legend.position.inside=c(.18,.945))
ggsave(filename = here("analysis","plots","crit_mean_props_collapsed_split_by_condition.jpeg"),width=6,height=6)
...
do_ranking_plot_data <- function(means, subs, dist){
  means <- means %>%
  ...
  p <- means %>%
    ggplot(aes(order,m))+
    geom_line(data=subs,aes(order,prop,group=sub_n),col="grey",alpha=.05,inherit.aes=F)+
    geom_line(aes(group=distance),col="black",alpha=.8)+
...
p_data <- map(list(2,5,9,14),
              do_ranking_plot_data,means=d_m_order,
              subs=d_sub_order) 
p_data[[1]]+p_data[[2]]+p_data[[3]]+p_data[[4]]+plot_layout(ncol=1,axis_titles = "collect")
ggsave(filename = here("analysis/plots/crit_ordering_data_means.jpeg"),width=4,height=8)

# ind sub analysis ===============================================================
d_att_choice_collapsed_by_dist_effects_ind_diffs <- d_att_choice_collapsed_by_dist %>%
  filter(option!="d") %>%
  pivot_wider(names_from = c(type,option), values_from = prop, values_fill = 0) %>%
  mutate(bc_minus_bt=best_c-best_t,
         wc_minus_wt=worst_c-worst_t,
         distance=factor(
           glue("{distance}% TDD"),
           levels=c("2% TDD",
                    "5% TDD",
...
d_att_choice_collapsed_by_dist_effects_means <- d_att_choice_collapsed_by_dist_effects_ind_diffs %>%
  group_by(distance) %>%
  summarise(
    bc_minus_bt=mean(bc_minus_bt),
    wc_minus_wt=mean(wc_minus_wt)
  ) %>%
  ungroup()

d_att_choice_collapsed_by_dist_effects_ind_diffs_percents <- d_att_choice_collapsed_by_dist_effects_ind_diffs %>%
  mutate(quadrant=case_when(
    wc_minus_wt < 0 & bc_minus_bt > 0 ~ "repulsion",
    wc_minus_wt > 0 & bc_minus_bt > 0 ~ "non-monotonic repulsion",
    wc_minus_wt > 0 & bc_minus_bt < 0 ~ "attraction",
    wc_minus_wt < 0 & bc_minus_bt < 0 ~ "non-monotonic attraction",
    wc_minus_wt==0 | bc_minus_bt==0 ~ "none",
  )) %>%
  group_by(distance, quadrant) %>%
  summarise(n=n()) %>%
  group_by(distance) %>%
  mutate(percent=format(round(100*n/sum(n),digits=2),nsmall=2),
         percent=glue("{percent}%")) %>%
  ungroup() %>%
  mutate(x=case_when(
    quadrant=="attraction"~.87,
    quadrant=="non-monotonic attraction"~-.87,
    quadrant=="repulsion"~-.87,
    quadrant=="non-monotonic repulsion"~.87,
    quadrant=="none"~50
  ),
  y=case_when(
    quadrant=="attraction"~-.1,
    quadrant=="non-monotonic attraction"~-.1,
    quadrant=="repulsion"~.1,
    quadrant=="non-monotonic repulsion"~.1,
    quadrant=="none"~50
  ))

d_att_choice_collapsed_by_dist_effects_ind_diffs %>%
  ggplot(aes(wc_minus_wt,bc_minus_bt))+
  geom_text(data=d_att_choice_collapsed_by_dist_effects_ind_diffs_percents,
            aes(x,y,label=percent),size=2,col='blue')+
  geom_point(alpha=.3,shape=1,size=.5)+
  geom_vline(xintercept=0,linetype="dashed")+
  geom_hline(yintercept=0,linetype="dashed")+
  geom_point(data=d_att_choice_collapsed_by_dist_effects_means,
             aes(wc_minus_wt,
                 bc_minus_bt),
             col="green",shape=4)+
  annotate(geom="text",
           x=.5,y=.5,
           label="non-\nmonotonic\nrepulsion",size=2,col="red")+
  annotate(geom="text",
           x=.5,y=-.5,
           label="attraction",size=2,col="red")+
  annotate(geom="text",
           x=-.5,y=.5,
           label="repulsion",size=2,col="red")+
  annotate(geom="text",
           x=-.5,y=-.5,
           label="non-\nmonotonic\nattraction",size=2,col="red")+
  labs(x="W(C)-W(T)",
       y="B(C)-B(T)")+
  coord_fixed(xlim=c(-1,1),ylim=c(-1,1))+
  facet_wrap(vars(distance),nrow=2,ncol=2)+
  ggthemes::theme_few()
ggsave(filename=here("analysis/plots/crit_indiv_diffs.jpeg"),
       width=4,height=4)
